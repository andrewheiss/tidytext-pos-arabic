...
> See the [actual blog post](https://www.andrewheiss.com/blog/2018/12/28/tidytext-pos-arabic/).
...
The [**stopwords** package](https://github.com/davnn/stopwords) also has a much of common Arabic words, accessible through `stopwords::stopwords(language = "ar", source = "misc"))`, but it's not as long of a list as `arabicStemR::removeStopWords()`, and it includes important Qur'anic words like يوم (day). So we'll just use the **arabicStemR** words here.
...
```{r most-common-nouns, fig.height=4}
...
```{r most-common-verbs, fig.height=4}
...
```{r plot-unique-nouns, fig.height=4}
